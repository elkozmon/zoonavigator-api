...
class ForceDeleteZNodeRecursiveCommandHandler(
  curatorFramework: CuratorFramework,
  backgroundPromiseFactory: BackgroundPromiseFactory,
  executionContextExecutor: ExecutionContextExecutor
) extends CommandHandler[ForceDeleteZNodeRecursiveCommand] {

  override def handle(command: ForceDeleteZNodeRecursiveCommand): Future[Unit] = {
