...
class JsonCofreeSpec extends FlatSpec with JsonCofree {
  ...
  "JsonCofree" should "serialize simple integer tree" in {
    val c = Cofree(1, Eval.now(List(Cofree(2, Eval.now(List.empty[Cofree[List, Int]])), Cofree(3, Eval.now(List.empty[Cofree[List, Int]])))))
  ...
  it should "deserialize simple integer tree" in {
    ...
    val e = Cofree(1, Eval.now(List(Cofree(2, Eval.now(List.empty[Cofree[List, Int]])), Cofree(3, Eval.now(List.empty[Cofree[List, Int]])))))
